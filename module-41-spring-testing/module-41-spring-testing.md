...
**(2) `@DynamicPropertySource`:** runs _after_ the WireMock extension starts (so
`wireMock.baseUrl()` is available) but _before_ the Spring context is created - this is the
